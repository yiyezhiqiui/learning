...
void pnp(cv::Mat &src, cv::Point2f (*points)[4], int &group_cnt, std::vector<cv::Vec3d> &output_position, std::vector<cv::Vec3d> &output_quater, int &type)
{
    ...
    std::vector<std::vector<cv::Point2f>> rect_points(contours.size() * 2, std::vector<cv::Point2f>(4));
    std::vector<std::vector<cv::Point2f>> points_tmp(contours.size() * 2, std::vector<cv::Point2f>(2));
    std::vector<std::vector<cv::Point2f>> point(contours.size() * 2, std::vector<cv::Point2f>(2));
    ...
    for (size_t i = 0; i < contours.size(); i++)
    {
        ...
        if (area > 20)
        {
            ...
            minRect[i].points(rect_points[i].data());
    ...
    std::vector<int> flag(contours_cnt, 0);
    ...
    for (int i = 0; i < group_cnt; i++)
    {
        ...
        std::ifstream cameraFile("./src/client_ros2/data/camera/matrix.txt");
        std::ifstream distFile("./src/client_ros2/data/camera/distcoeffs.txt");
